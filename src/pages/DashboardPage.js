...
export default function DashboardPage() {
  ...
  const statCards = [
    { value: halaqahs.length, label: 'حلقاتي', gradient: 'gradient-card-green', color: 'text-primary-600', iconBg: 'bg-primary-100',
  ...
  if (user.role === 'teacher') {
    statCards.push({ value: stats.totalStudents, label: 'الطلاب', gradient: 'gradient-card-blue', color: 'text-blue-600', iconBg: 'bg-blue-100',
  ...
  statCards.push({ value: stats.totalSessions, label: 'الجلسات', gradient: 'gradient-card-gold', color: 'text-yellow-600', iconBg: 'bg-yellow-100',
    icon: <svg className="w-5 h-5 text-yellow-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg> });
  if (user.role === 'student') {
    statCards.push({ value: stats.totalRecitations, label: 'تسميعاتي', gradient: 'gradient-card-purple', color: 'text-purple-600', iconBg: 'bg-purple-100',
      icon: <svg className="w-5 h-5 text-purple-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" /></svg> });
  }
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 mb-8">
        ...
        {halaqahs.length === 0 && (
          <div className="col-span-full empty-state">
            ...
            <p className="text-gray-400">لا توجد حلقات بعد</p>
            <p className="text-gray-300 text-sm mt-1">{user.role === 'teacher' ? 'أنشئ حلقتك الأولى' : 'انضم لحلقة من صفحة الحلقات'}</p>
          </div>
        )}
      </div>
      ...
      {user.role === 'student' && recitations.length > 0 && (
        <div className="animate-fade-in-up">
          <h2 className="text-lg font-bold text-gray-800 mb-4">تسميعاتي ({recitations.length})</h2>
          <div className="bg-white rounded-xl shadow-sm border p-6">
            <div className="space-y-3">
              {recitations.map((r) => (
                <div key={r.id} className="flex justify-between items-center border rounded-lg p-4 card-hover">
                  <div>
                    <p className="font-medium text-gray-800">{getSurahName(r.surahNumber)} - آية {r.fromAyah} إلى {r.toAyah}</p>
                    <div className="flex items-center gap-2 mt-1">
                      ...
                      <span className="text-xs text-gray-400">{new Date(r.createdAt).toLocaleDateString('ar')}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2">
                    {r.evaluation && (
                      <div className="flex gap-1">
                        <span className={`text-[10px] px-1.5 py-0.5 rounded font-bold ${r.evaluation.hifdh >= 8 ? 'text-green-600 bg-green-50' : r.evaluation.hifdh >= 5 ? 'text-yellow-600 bg-yellow-50' : 'text-red-600 bg-red-50'}`}>{r.evaluation.hifdh}/10</span>
                        <span className={`text-[10px] px-1.5 py-0.5 rounded font-bold ${r.evaluation.tajweed >= 8 ? 'text-green-600 bg-green-50' : r.evaluation.tajweed >= 5 ? 'text-yellow-600 bg-yellow-50' : 'text-red-600 bg-red-50'}`}>{r.evaluation.tajweed}/10</span>
                        <span className={`text-[10px] px-1.5 py-0.5 rounded font-bold ${r.evaluation.fluency >= 8 ? 'text-green-600 bg-green-50' : r.evaluation.fluency >= 5 ? 'text-yellow-600 bg-yellow-50' : 'text-red-600 bg-red-50'}`}>{r.evaluation.fluency}/10</span>
                      </div>
                    )}
                    <Link to={`/sessions/${r.sessionId}`} className="text-sm text-primary-600 hover:underline">تفاصيل الجلسة</Link>
                  </div>
                </div>
              ))}
            </div>
