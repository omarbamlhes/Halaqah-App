    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="flex gap-2 mb-6">
        <button
          ...
          className={`px-4 py-2 rounded-lg font-medium transition ${
            tab === 'my' ? 'bg-primary-600 text-white' : 'bg-gray-200 text-gray-600'
          }`}
        >
        ...
        {user.role === 'student' && (
          <button
            ...
            className={`px-4 py-2 rounded-lg font-medium transition ${
              tab === 'public' ? 'bg-primary-600 text-white' : 'bg-gray-200 text-gray-600'
            }`}
          >
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {(tab === 'my' ? halaqahs : publicHalaqahs).map((h) => (
          <div key={h.id} className="bg-white rounded-xl shadow-sm border p-6 card-animated card-hover">
            <Link to={`/halaqahs/${h.id}`}>
              <h3 className="text-lg font-bold text-gray-800 mb-2">{h.name}</h3>
              <p className="text-gray-500 text-sm mb-3">{h.description || 'بدون وصف'}</p>
              <p className="text-xs text-gray-400">المعلم: {h.teacher?.name}</p>
            </Link>
            {tab === 'public' && user.role === 'student' && !myIds.includes(h.id) && (
              <button
                onClick={() => handleJoin(h.id)}
                className="mt-3 w-full bg-primary-50 text-primary-700 py-2 rounded-lg font-medium hover:bg-primary-100 transition"
              >
                انضمام
              </button>
            )}
